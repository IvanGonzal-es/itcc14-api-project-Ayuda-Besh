...
@admin_required
def get_all_users():
    """Admin endpoint to view all users with filtering options"""
    try:
        db = get_database()
        
        # Get filter parameters
        role_filter = request.args.get('role')  # 'customer', 'provider', or None for all
        search_query = request.args.get('search', '').strip()  # Search by username or fullName
        
        # Build query
        query = {}
        if role_filter and role_filter in ['customer', 'provider', 'admin']:
            query['role'] = role_filter
        
        # Get all users matching role filter (exclude passwords)
        users = list(db.users.find(query, {'password': 0}).sort('createdAt', -1))
        ...
        if search_query:
            ...
            users = [
                ...
                if search_lower in u.get('username', '').lower() or
                   search_lower in u.get('fullName', '').lower() or
                   search_lower in u.get('email', '').lower()
            ]
        
        # Format response - ensure sensitive data is handled properly
        for user in users:
            user['_id'] = str(user['_id'])
            # Ensure password is never included (already excluded in query, but double-check)
            if 'password' in user:
                del user['password']
            ...
            if 'verified_at' in user and user.get('verified_at'):
                user['verified_at'] = user['verified_at'].isoformat() if hasattr(user['verified_at'], 'isoformat') else str(user['verified_at'])
...
def get_user_details(user_id):
    ...
    try:
        ...
        if user.get('role') == 'provider':
            user['services_offered'] = user.get('services_offered', [])
            user['location'] = user.get('location', 'Not specified')
            user['description'] = user.get('description', 'No description provided')
            user['hourly_rate'] = user.get('hourly_rate', 0)
            user['service_radius'] = user.get('service_radius', 0)
            user['equipment'] = user.get('equipment', 'Not specified')
            user['rating'] = user.get('rating', 0)
...
@admin_required
def create_user():
    """Admin-only endpoint to create user accounts (including admin accounts)"""
    try:
        ...
        if role == 'provider':
            user_doc['is_verified'] = data.get('is_verified', False)
            user_doc['services_offered'] = data.get('services_offered', [])
            user_doc['location'] = data.get('location', '')
            user_doc['description'] = data.get('description', '')
            user_doc['hourly_rate'] = data.get('hourly_rate', 0)
            user_doc['service_radius'] = data.get('service_radius', 0)
            user_doc['equipment'] = data.get('equipment', '')
            user_doc['rating'] = 0
